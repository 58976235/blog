<template>
  <div>
    <a-menu v-model="current" mode="inline"  @select="selectItem">
      <template v-for="route in routerList">
        <a-menu-item :key="route.path" v-if="!route.children.length" class="nav-item" @click="menuActive">
          <span><a-icon :type="route.icon" theme="filled" />{{ route.title }}</span>
        </a-menu-item>
        <a-sub-menu :key="route.name" v-else class="nav-item">
          <span slot="title" class="submenu-title-wrapper" >
            <a-icon :type="route.icon"  theme="filled" />{{ route.title }}</span
          >
          <a-menu-item :key="children.path" v-for="children in route.children" class="children-item" @click="menuActive">
            {{ children.title }}
          </a-menu-item>
        </a-sub-menu>
      </template>
